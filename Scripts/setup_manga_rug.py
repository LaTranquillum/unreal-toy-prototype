"""Use the six source panels as non-colliding grayscale floor artwork."""
import unreal,json
...
def material(i,r):
 ...
 sample=expr(m,unreal.MaterialExpressionTextureSample,-500,0);sample.set_editor_property('texture',texture)
...
panel('charcoal backing',-290,-270,.6,560,780,base)
for i,r in enumerate(regions):
 col=i%2;row=i//2
 x=-430+col*280;y=-10-row*260
 aspect=((r[3]-r[1])*1297)/((r[4]-r[2])*1920)
 w=min(258,238*aspect);h=w/aspect
 panel(str(i+1)+' '+r[0],x,y,.9,w,h,mats[i])
...
report={'manga_rug_pass':True,'source':'Reference/2DT.jpeg','panel_count':6,'grayscale':True,'non_colliding':True,'original_overlaps_preserved':True,'regions':regions}
(Path(unreal.Paths.project_saved_dir())/'Verification/manga_rug.json').write_text(json.dumps(report,indent=2))
unreal.log('MANGA RUG PASS')
